...
public class RemoveCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (player.hasPermission("vcr.admin")) {
			if (args.length != 1) {
				player.sendMessage("§cWrong use , please try to type /crremove <reason>");
				return false;
			} else {
				String reason = args[0];
				if (Configuration.isExists(reason)) {
					Configuration.removeReason(reason);
					player.sendMessage("§cYou have been removed this §6" + reason + " §creason successfully !");
					player.playSound(player.getLocation(), Sound.ORB_PICKUP, 0.5f, 5.0f);
					Configuration.save();
				} else {
					player.sendMessage("§cThis §f" + reason + "§c reason not exists !");
